<x-app-layout>
    <x-slot name="header">
        ...
        <h6 class="mb-0 font-bold text-dark capitalize">Tạo công thức</h6>
    </x-slot>
            ...
            <div class="flex flex-wrap mx-10 mb-6 gap-y-6">
                    ...
                    <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl rounded-2xl bg-clip-border">
                        <div class="flex-auto p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="summary" name="summary" rows="3" class="focus:shadow-primary-outline text-sm ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500">{{ old('summary') }}</textarea>
                            ...
                            <div>
                                ...
                                <select id="ingredient_ids" name="ingredient_ids[]" multiple size="8" class="focus:shadow-primary-outline text-sm ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500">
                                    @foreach ($ingredientsCatalog as $ingredient)
                                        <option value="{{ $ingredient->id }}" @selected(collect(old('ingredient_ids', []))->contains($ingredient->id))>{{ $ingredient->name }}</option>
                                    @endforeach
                                </select>
                                <p class="mt-1 text-xs text-gray-500">Giữ Ctrl hoặc Command để chọn nhiều nguyên liệu.</p>
                                @error('ingredient_ids') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                                @error('ingredient_ids.*') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="ingredients" class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700">Chi tiết nguyên liệu</label>
                                <textarea id="ingredients" name="ingredients" rows="5" required class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500">{{ old('ingredients') }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea id="instructions" name="instructions" rows="6" required class="focus:shadow-primary-outline text-sm ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500">{{ old('instructions') }}</textarea>
                ...
                <div class="w-full max-w-full md:px-3 shrink-0 md:w-4/12 md:flex-0">
                    <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl rounded-2xl bg-clip-border">
                        <div class="flex-auto p-6 space-y-4">
                            <div>
                                <label for="image" class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700">Hình ảnh</label>
                                <input type="file" id="image" name="image" required class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500" />
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" id="prep_time" name="prep_time" min="0" value="{{ old('prep_time') }}" class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500" />
                                </div>
                                <div>
                                    <label for="cooking_time" class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700">Nấu (phút)</label>
                                    <input type="number" id="cooking_time" name="cooking_time" min="1" value="{{ old('cooking_time') }}" required class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500" />
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" id="servings" name="servings" min="1" value="{{ old('servings') }}" class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500" />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <select id="dish_type_id" name="dish_type_id" class="focus:shadow-primary-outline text-sm ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500">
                                    ...
                                    @foreach ($dishTypes as $dishType)
                                        <option value="{{ $dishType->id }}" @selected(old('dish_type_id') == $dishType->id)>{{ $dishType->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="url" id="video_url" name="video_url" value="{{ old('video_url') }}" class="focus:shadow-primary-outline text-sm leading-5.6 ease block w-full rounded-lg border border-solid border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none transition-all focus:border-blue-500" />
